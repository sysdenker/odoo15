...
class SaleOrder(models.Model):
    ...
    def _cart_update_uom(self, product_id=None, line_id=None, add_qty=0, set_qty=0, uom_id = 0, **kwargs):
        ...
        print("add_qty", add_qty, "set_qty", set_qty)
        uom_factor = 1
        if uom_id:
            product_uom_id = uom_id
            uom = self.env['uom.uom'].sudo()
            uom_id = uom.browse(int(product_uom_id))
            uom_factor = uom_id.factor_inv

        try:
            if add_qty:
                add_qty = int(add_qty)
                add_qty = add_qty / uom_factor
        except ValueError:
            add_qty = 1
        try:
            if set_qty:
        ...
        if not order_line:
            ...
            if uom_id.id != product.uom_id:
                values['product_uom'] = uom_id.id
        ...
        if quantity <= 0:
            ...
            if uom_id and uom_id.id != order_line.product_id.uom_id:
                values['product_uom'] = uom_id.id
                # values['product_uom_qty'] = quantity / uom_id.factor_inv
    ...
    def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0, **kwargs):
        ...
        for order in self:
            line_id = order._cart_find_product_line(product_id, line_id, **kwargs)
            if line_id:
                if line_id.product_uom.id != line_id.product_id.uom_id.id:
                    line_id.price_unit = line_id.price_unit * line_id.product_uom.factor_inv
        return res
