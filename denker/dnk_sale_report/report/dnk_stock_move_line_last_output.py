...
class DnkStockMoveLastOutput(models.Model):
    _name = "dnk.stock.move.line.last.output"
    _description = "Denker Stock Move Last Output"
    _auto = False
    _rec_name = 'date'
    _order = 'date desc'
    ...
    def _query(self, with_clause='', fields={}, groupby='', from_clause='', where=''):
        ...
        where_ = """
            ...
            AND sml.x_studio_location_type_origin <> 'customer'
        """

        return '%s (SELECT %s FROM %s WHERE %s GROUP BY %s)' % (with_, select_, from_, where_, groupby_)

    def init(self):
        self._table = "dnk_stock_move_line_last_output"
        tools.drop_view_if_exists(self.env.cr, self._table)
        self.env.cr.execute("""CREATE or REPLACE VIEW %s as (%s)""" % (self._table, self._query()))


class DnkProductCatalogProforma(models.AbstractModel):
    _name = 'dnk.stock.move.line.last.output.proforma'
    _description = 'Denker Stock Move Last Output Report'

    def _get_report_values(self, docids, data=None):
        docs = self.env['stock.move.line'].browse(docids)
        return {
                'doc_ids': docs.ids,
                'doc_model': 'dnk.stock.move.line.last.output',
                'docs': docs,
                'proforma': True
            }
