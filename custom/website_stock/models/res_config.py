...
class WebsiteStockConfigSettings(models.Model):
    ...
    wk_stock_type = fields.Selection([('on_hand', 'Quantity on Hand'), ('forecasted', 'Forecasted Quantity'), (
        'outgoing', 'Quantity On Hand - Outgoing Quantity')], string='Stock Type', required="1", default="on_hand",  ondelete='cascade')

    wk_warehouse_type = fields.Selection([('all', 'ALL'), ('specific', 'SPECIFIC')], string="Default Stock Location", required="1",  ondelete='cascade', default="all")
