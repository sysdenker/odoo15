...
class SaleOrder(models.Model):
    ...
    def check_constraint_quantity(self):
        if self.env.user.has_group('abs_so_minimum_quantity.minimum_quantity_group'):
            return
        for record in self:
...
class SaleOrderLine(models.Model):
    ...
    def product_id_change(self):
        if self.product_id.minimum_order_quantity > 0:
            self.product_uom_qty = self.product_id.minimum_order_quantity
