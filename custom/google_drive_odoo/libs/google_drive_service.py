...
class GoogleDriveApiClient():
    """
    Wrapper for API requests to Google Drive
    """
    ...
    def __init__(self, client_id, client_secret, cloud_client):
        """
        Init method
         ...
         * client_secret - char
         * client_secret - char
         * cloud_client object (for logging)
        """
    ...
    def get_token_from_refresh_token(self, refresh_token, redirect_uri):
        ...
        Methods:
         * _cloud_log of clouds.client: need to control response, since it might be without any error
        ...
        headers = {"Content-type": "application/x-www-form-urlencoded"}
        self.clouds_client._cloud_log(True, "Start receiving refresh token", "DEBUG")
        response = requests.post(self.GOOGLE_TOKEN_ENDPOINT, data=params, headers=headers, timeout=self.TIMEOUT)
        content = response.json()
        # not to log access token explicitly
        content_log = content.copy()
        if content_log.get("access_token"):
            content_log["access_token"] = "***************************************"
        self.clouds_client._cloud_log(True, "Token from refresh token response: {}".format(content_log), "DEBUG")
    ...
    def _get_file_metadata(self, drive_id):
        ...
        if response and response.get("mimeType"):
            odoo_mimetype = self._correspond_gdrive_mimetypes(response.get("mimeType"))
            response.update({"mimeType": odoo_mimetype})
        return response
    ...
    def _create_folder(self, name, parent):
        ...
        if (not parent or parent == "root") and self.team_drive_id:
            self.clouds_client._cloud_log(True, "Team Drive ID is assigned".format(self.team_drive_id), "DEBUG")
        ...
        if self.team_drive_id:
            file_metadata.update({"driveId": self.team_drive_id})
    ...
    def _list_children_items(self, drive_id):
         ...
         * drive_id - Google Drive ID
        """
        self._get_file_metadata(drive_id)
        ...
        params = {
            ...
            "pageSize": 1000,
            "pageToken": None,
            "fields": "kind,nextPageToken,files(id,name,webViewLink,webContentLink,mimeType)",
        }
        if self.team_drive_id:
            params.update({
                "corpora": "teamDrive",
    ...
    def _download_file(self, drive_id):
        ...
        try:
            ...
            # for google drive docs: UP TO 10MB
            ...
            res = self._request_gd(method="GET", url=url, params=params, timeout=self.TIMEOUT, headertoken=True)
        return res and res._content or False
