...
const fileKanbanController = KanbanController.extend(fileUploadMixin, {
    ...
    events: _.extend({}, KanbanController.prototype.events, {
        ...
        "click #file_manager_update_tags": "_onUpdateTags",
    ...
    */
     ...
     * @private
     * @param {MouseEvent} event
     * The method to bulk edit article tags by opening a wizard
    */
    async _onUpdateTags(event) {
        event.stopPropagation();
        var self = this;
        var onSaved = function(record) {self.reload()};
        new dialogs.FormViewDialog(self, {
            res_model: "mass.update.tag",
            context: {'default_attachments': self.selectedRecords.join()},
            title: _lt("Update tags"),
            readonly: false,
            shouldSaveLocally: false,
            on_saved: onSaved,
        }).open();
