...
class clouds_queue(models.Model):
    ...
    def _is_any_parent_blocking(self):
        ...
        while parent_id:
            for task in parent_id.task_ids:
                if task.active and task.sequence < 100:
                    return True
    ...
    def _is_task_blocked(self):
        ...
        if self.task_type != "reverse" and "_reverse" not in self.name and self._is_any_parent_blocking():
            # IMPORTANT: parents do not block reverse and backward tasks during reverse
        ...
        if prev_folder_id and prev_folder_id != self.folder_id and self.attachment_id:
            for prev_task in prev_folder_id.task_ids:
                if self._relates_to_task_attachment(self.attachment_id, self.attachment_id.cloud_key):
                    return True
        return False
    ...
    def _get_the_next_tasks(self):
        ...
        basic_domain = [("next_attempt", "<=", fields.Datetime.now()), ("active", "=", True)]
        reverse_domain = basic_domain + [
            "|", "|", ("name", "=", "move_file"), ("name", "ilike", "_reverse"), ("task_type", "=", "reverse")
        ]
        has_reverse = self.search(reverse_domain, limit=1)
        if has_reverse:
            next_tasks = _get_tasks(reverse_domain, self.env["clouds.queue"], 0)
        else:
            next_tasks = _get_tasks(basic_domain, self.env["clouds.queue"], 0)
        return next_tasks
