        ...
        if "client_id" in folder_changes:
            ...
            if new_client_id:
                if new_back_state is False:
                    ...
                    _initial_setup()
                elif not new_state.get("cloud_key") and not pre_state.get("cloud_key") and new_state.get("active") \
                        and (not self.task_ids or not self.task_ids.filtered(lambda ta: ta.name == "setup_sync")):
                    # we didn't not setup folder but should do so. It represents a very rare case when folder was
                    # removed, and the initial_setup failed 8 times. The same as previous basically
                    _initial_setup()
                else:
